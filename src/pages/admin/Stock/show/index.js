...
import { alertConfirmation } from "../../../../components/sweet-alert";
...
const Stock = () => {
  ...
  const handleDelete = () => {
    alertConfirmation(
      "Etes vous sures de vouloir supprimer le stock , cette action sera irreversible",
      ({ isConfirmed }) => {
        if (isConfirmed) {
          console.log("delete");
        }
  ...
  return (
    <>
      <PageHeader title={`Details du stock ${state?.numero}`}>
        <div className="offset-sm-7 col-sm-8">
          ...
          <Button
            ...
            onClick={() =>
              navigate("/motors/add", {
                state: { id: state?.id, numero: state?.numero },
              })
            }
          >
            Ajouter une moto
