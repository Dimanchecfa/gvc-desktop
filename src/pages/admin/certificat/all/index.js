...
import { certifiedMotors, fetchMotorsCertified } from "../../../../api/request";
...
const MotorsCertified = () => {
  ...
  const handleSubmit = async () => {
    setShowModal(false);
    try {
      const response = await certifiedMotors(array_uncertified.current);
      fetchMotorsCertifiedData();
      successNotif("Enregistrement reussi");
  ...
  return (
    <>
      <PageHeader title="Listes des certication de mise en circulation">
      ...
      <MotorsUncertifiedModal
        show={showModal}
        handleClose={() => setShowModal(false)}
        handleSelect={(row) => handleSelected(row)}
